...
// Set maxCategories so features with > 4 distinct values are treated as continuous.
        System.out.println("Select the prediction/label column");
        String[] arr={"imonth","iday","country_txt","region_txt","city","success","suicide","attacktype1_txt","natlty1_txt","weaptype1_txt","gname","targtype1_txt"};
        ...
        int val=s.nextInt();
        while(val>arr.length ||val<1)
        {
            System.out.println("Invalid option");
            val=s.nextInt();
        }
        String[] arr_indexed={"imonth","iday","country_txt_indexed","region_txt_indexed","city_indexed","success","suicide","attacktype1_txt_indexed","natlty1_txt_indexed","weaptype1_txt_indexed","gname_indexed"};
//
        for(int i=0;i<arr.length;i++)
        {
            if(i==val-1)
            {
                StringIndexer indexer=new StringIndexer().setInputCol(arr[i]).setOutputCol("label").setHandleInvalid("skip");
                data=indexer.fit(data).transform(data);
                continue;
            }
            StringIndexer indexer1 = new StringIndexer().setInputCol(arr[i]).setOutputCol(arr[i] + "_indexed").setHandleInvalid("skip");
...
//        StringIndexer indexer9 = new StringIndexer().setInputCol("targtype1_txt").setOutputCol("label").setHandleInvalid("skip");
...
//        data_indexed.withColumn("country_txt" + "_indexed", (Column) lit(1));
        Dataset<Row> data_indexed=data.select("imonth","iday","country_txt_indexed",
                ...
                "natlty1_txt_indexed","weaptype1_txt_indexed","gname_indexed","label");
...
// Select example rows to display.
        ...
        predictions.select("label","prediction").coalesce(1).
                write().
                format("com.databricks.spark.csv").
                option("header", "true").
                save("src/main/resources/prediction.csv");
...
//        Dataset<Row> f=converter.transform(testData);
